...
$id = $_GET['id'];
$where = ['id' => $id];
$row = $member->readData("WHERE id = $id");
...
if(isset($_POST['submit'])){
    ...
    elseif(!preg_match('/^08\d{8,18}$/', $_POST['no_telp'])) {
        ...
        $member->updateData($data,$where);
        header('Location: dataMember.php?success=true&message=Data berhasil diedit.');
    }
}
?>
          ...
          <div class="container mt-3">
            <form class="form" method="POST" action="" enctype="multipart/form-data">
                <div class="form-group mb-3">
                    ...
                    <?php endif; ?>
                      <input name="nama" value="<?= $row[0]['nama']; ?>" class="form-control" id="nama" type="text" placeholder="Ketikan Nama Member...">
                    </div>
                </div>
                <div class="form-group mb-3">
                  ...
                  <div>
                    <?php if(isset($error_kolom) && $error_kolom == "email"): ?>
                        <div class="alert alert-danger"><?php echo $error_message; ?></div>
                    <?php endif; ?>
                    <input name="email" value="<?= $row[0]['email']; ?>" class="form-control" type="email" placeholder="Ketikan Email Member...">
                  </div>
                </div>
                <div class="form-group mb-3">
                  ...
                  <div>
                    <?php if(isset($error_kolom) && $error_kolom == "no_telp"): ?>
                        <div class="alert alert-danger"><?php echo $error_message; ?></div>
                    <?php endif; ?>
                    <input name="no_telp" value="<?= $row[0]['no_telp']; ?>" class="form-control" type="number" placeholder="Ketikan Email Member...">
                  </div>
                </div>
                  <div class="form-group mb-3">
                    ...
                    <div>
                      <?php if(isset($error_kolom) && $error_kolom == "alamat"): ?>
                          <div class="alert alert-danger"><?php echo $error_message; ?></div>
                      <?php endif; ?>
                      <textarea name="alamat" class="form-control" type="number" placeholder="Ketikan Alamat Member..."><?= $row[0]['alamat']; ?></textarea>
                    </div>
                  </div>
                <div class="tright">
